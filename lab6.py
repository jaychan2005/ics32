...
def run(path, file):
    ...
    if note == ":d":
        try:
            note = mod.remove_note(path, file)
            if note is None:
        ...
        mod.save_note(note, path, file)
    run(path, file)


if __name__ == "__main__":
    assert mod.is_int(5) == True
    assert mod.is_int("five") == False

    print("Welcome to PyNote! \n")
    path = "."
    file = "pynote.txt"
    mod.read_notes(path, file)
    run(path, file)
    
###################################################################################
# TODO: PASTE YOUR SECOND MODULE CODE BELOW BEFORE SUBMITTING!
###################################################################################
# name of module is 'mod.py'
from pathlib import Path
